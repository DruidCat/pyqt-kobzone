...
понравится, и принесёт пользу.</p>
            ...
            txdZona.text = qsTr("
                <html>
                    <body>
<p>На данной странице вы можете изменить основные параметры отображения интерфейса и получить техническую \
информацию о приложении.</p>
<p><b>НАСТРОЙКИ И ИНФОРМАЦИЯ:</b></p>
<ol>
<p><b>[журнал]</b> - Просмотр системных сообщений и событий работы приложения. Все данные журнала \
фиксируются и сохраняются в файл <b>logs.txt</b>.</p>
<p><b>[шрифт]</b> - Изменение размера шрифта во всем интерфейсе приложения (маленький, средний, большой).</p>
<p><b>[горячие клавиши]</b> - Открыть страницу с подробной инструкцией по всем комбинациям клавиш для \
быстрого управления программой.</p>
<p><b>[о qt]</b> - Информация о лицензии и правообладателях фреймворка Qt, на базе которого разработано \
приложение.</p>
</ol>
<p><b>ФУНКЦИОНАЛ:</b></p>
<ol>
...
(Alt+стрелка влево).</p>
<p><img src = \"qrc:/resources/images/DCButtons/24x24/DCKnopkaMenu.png\"> - Настройка LM Studio.</p>
<p><img src = \"qrc:/resources/images/DCButtons/24x24/DCKnopkaSidebar_3.png\"> - Настройка нейро анализа.</p>
...
<p><b>[Содержимое документов:]</b> - Здесь отобразится содержимое тех документов, которые вы выбрали на \
анализ языковой модели.</p>
<p><b>[📦 добавить RAG БД]</b> - Выберите RAG базу данных для контекста или поиска информации из неё.</p>
...
<p><img src = \"qrc:/resources/images/DCButtons/24x24/DCKnopkaZakrit.png\"> - Удаление текста промта.</p>
...
<p><b>[💾 сохранить результат]</b> - Вы можете сохранить результат анализа в текстовом файле.(Ctr+S).</p>
			...
			txdZona.text = qsTr("
				<html>
					<body>
<p>На данной странице вы можете настроить качество, глубину и скорость анализа документов с помощью \
локальных языковых моделей (LLM).</p>
<p><b>НАСТРОЙКА ПАРАМЕТРОВ АНАЛИЗА:</b></p>
<ul>
<li><b>[температура]</b> — параметр, управляющий уровнем «креативности» и детерминированности ответов \
модели. Для задач анализа документов, где важна фактическая точность, рекомендуются низкие значения \
(<b>0.1 – 0.3</b>). Значение <b>0.5</b> является сбалансированным. Чем выше значение, тем более вариативны \
ответы (возрастает риск «галлюцинаций»); чем ниже — тем строже, точнее и предсказуемее результат.</li>
<li><b>[перекрытие]</b> — процент текста, который дублируется в соседних фрагментах \
(чанках) при разделении документа. Увеличение перекрытия повышает качество анализа (сохраняет логический \
контекст), но пропорционально замедляет скорость обработки и увеличивает расход токенов.<br>
&nbsp;&nbsp;• <b>0%</b> ⭐⭐ (60% качества): Чанки полностью изолированы, границы рвут контекст. Если \
ключевая мысль разделена между чанками, модель её не увидит. Не рекомендуется.<br>
&nbsp;&nbsp;• <b>10%</b> ⭐⭐⭐ (75% качества): Минимальная защита от разрыва мысли. Подходит для простых \
текстов с чёткой структурой (новости, FAQ, короткие инструкции).<br>
&nbsp;&nbsp;• <b>20%</b> ⭐⭐⭐⭐ (85% качества): <b>Оптимальный выбор</b> для большинства задач. \
Предложения на границах разделения полностью попадают в соседний чанк, сохраняя связность.<br>
&nbsp;&nbsp;• <b>30%</b> ⭐⭐⭐⭐⭐ (92% качества): Рекомендуется для сложных текстов (научные статьи, \
юридические документы, технические спецификации). Гарантирует, что целые абзацы не будут разорваны.<br>
&nbsp;&nbsp;• <b>40%</b> ⭐⭐⭐⭐⭐ (94% качества): Максимальное сохранение контекста, но ведёт к \
значительному увеличению времени обработки и нагрузки на память. Используйте только для критически важных \
задач.</li>
<li><b>[Финальный промт]</b> — системный запрос, который применяется на заключительном этапе обработки. Если \
документ был разделён на несколько чанков, этот промт инструктирует модель о том, как грамотно объединить, \
синтезировать и структурировать результаты анализа отдельных фрагментов в единый, связный итоговый отчёт без \
повторений.</li>
</ul>
<p><b>ФУНКЦИОНАЛ:</b></p>
<ul>
<p><img src='qrc:/resources/images/DCButtons/24x24/DCKnopkaNazad.png' width='24' height='24'> — \
<b>Назад</b>: вернуться в предыдущее меню (или нажмите <i>Alt + стрелка влево</i>).</p>
<p><img src='qrc:/resources/images/DCButtons/24x24/DCKnopkaInfo.png' width='24' height='24'> — \
<b>Помощь</b>: открыть справочную информацию (или нажмите <i>F1</i>).</p>
</ul>
<p><font color='#d32f2f'><b>⚠️ Обратите внимание:</b></font> Если LM Studio не запущена или языковая модель \
не выбрана, процесс анализа документов не будет запущен.</p>
					</body>
				</html>
			"); 
			...
			txdZona.text = qsTr("
				<html>
					<body>
<p>На данной странице вы можете настроить параметры взаимодействия приложения с локальным сервером \
нейросетей <b>LM Studio</b>.</p>
<p><b>УПРАВЛЕНИЕ LM STUDIO:</b></p>
<ul>
<li><b>[запустить LM Studio]</b> — инициирует запуск приложения LM Studio и активирует работу локального \
API-сервера.</li>
<li><b>[остановить LM Studio]</b> — корректно закрывает приложение LM Studio и останавливает сервер.</li>
<li><b>[путь к LM Studio]</b> — позволяет указать полный путь к исполняемому файлу основного приложения \
LM Studio на вашем компьютере.</li>
<li><b>[путь к lms]</b> — указывает путь к исполняемому файлу утилиты командной строки (CLI) <i>lms</i>, \
которая поставляется вместе с LM Studio. Она используется вашим приложением для автоматизированного \
управления сервером и моделями в фоновом режиме, без необходимости открывать графический интерфейс.</li>
<li><b>[выбор модели]</b> — позволяет выбрать конкретную языковую модель из списка тех, что были \
предварительно загружены в ваше хранилище LM Studio.</li>
<li><b>[максимальный контекст]</b> — определяет размер контекстного окна модели (объём текста в токенах), \
который нейросеть одновременно удерживает в памяти при формировании ответа: ваш запрос, переданные фрагменты \
документов и генерируемый ответ. Чем больше значение, тем больше информации модель обрабатывает за раз, что \
повышает точность, но увеличивает нагрузку на оперативную память и снижает скорость генерации. Рекомендуемое \
значение — <b>8000</b> токенов. При ошибках нехватки памяти (OOM) уменьшите этот параметр; при анализе \
объёмных документов — увеличьте.</li>
<li><b>[gpu offload]</b> (от 0 до 100%) — определяет, какая часть вычислительных слоёв нейросети будет \
загружена в видеопамять (VRAM) для ускорения работы. Значение <b>100%</b> обеспечивает максимальную \
скорость, но требует достаточного объёма VRAM. Если видеопамяти недостаточно, уменьшите этот параметр: \
недостающие слои будут обработаны центральным процессором (CPU), что предотвратит сбои, но намного снизит \
скорость.</li>
<li><b>[адрес сервера]</b> — сетевой адрес локального API, к которому ваше приложение будет отправлять \
запросы. По умолчанию используется <b>http://localhost:1234</b>. Убедитесь, что в настройках LM Studio \
включен локальный сервер (Local Server) и указан тот же порт.</li>
</ul>
<p><b>НАВИГАЦИЯ И ФУНКЦИОНАЛ:</b></p>
<ul>
<p><img src='qrc:/resources/images/DCButtons/24x24/DCKnopkaNazad.png' width='24' height='24'> — \
<b>Назад</b>: вернуться в предыдущее меню (или нажмите <i>Alt + стрелка влево</i>).</p>
<p><img src='qrc:/resources/images/DCButtons/24x24/DCKnopkaInfo.png' width='24' height='24'> — \
...
<p><font color='#d32f2f'><b>⚠️ Обратите внимание:</b></font> Для корректной работы процесса анализа \
необходимо, чтобы приложение LM Studio было запущено, а выбранная языковая модель была полностью загружена \
в память.</p>
					</body>
				</html>"); 
...
модели.</p>
    		...
    		txdZona.text = qsTr("
        		<html>
            		<body>
<p>На данной странице вы можете настроить параметры создания и работы с RAG (Retrieval-Augmented Generation) \
базами данных для поиска релевантных фрагментов документов.</p>
<p><b>НАСТРОЙКИ RAG:</b></p> 
<p><b>[gpu вкл/выкл]</b> Переключает режим обработки между видеокартой (GPU) и процессором (CPU). \
Режим GPU значительно ускоряет создание и поиск в базе данных, но требует совместимой видеокарты с \
поддержкой CUDA. Режим CPU работает медленнее, но не требует специального оборудования. Если возникают \
ошибки или программа зависает — переключитесь на CPU.</p>
<p><b>[модель]</b> Выбор модели эмбеддингов для преобразования текста в числовые векторы. От выбора \
модели зависит качество поиска, скорость работы и поддержка языков:</p>
<ul>
<li><b>all-MiniLM-L6-v2 (384D, быстрая)</b> — самая быстрая модель с минимальными требованиями к памяти. \
Подходит для простых задач и слабых компьютеров.</li>
<li><b>all-MiniLM-L12-v2 (384D, точная)</b> — улучшенная версия предыдущей модели с более точным поиском \
при тех же требованиях к ресурсам.</li>
<li><b>paraphrase-multilingual (384D, многоязычная)</b> — поддерживает более 50 языков, включая русский. \
Рекомендуется как золотая середина для многоязычных документов.</li>
<li><b>all-mpnet-base-v2 (768D, высокое качество)</b> — обеспечивает отличное качество поиска. \
Требует больше памяти и работает медленнее, чем 384D модели.</li>
<li><b>LaBSE (768D, 100+ языков)</b> — поддерживает более 100 языков с высоким качеством. Идеальна для \
документов на редких языках, но требует значительных ресурсов.</li>
<li><b>BAAI/bge-m3 (1024D, мультиязычная 8K)</b> — современная мощная модель с поддержкой длинных \
контекстов (до 8192 токенов). Отлично понимает множество языков, но требует больше вычислительных ресурсов.</li>
<li><b>intfloat/multilingual-e5-large (1024D, максимум для GPU)</b> — одна из лучших моделей для \
многоязычного поиска с высочайшей точностью. Оптимально и комфортно работает только при наличии мощной видеокарты.</li>
</ul>
<p><b>[batch GPU]</b> (от 1 до 256) Определяет, сколько текстовых фрагментов обрабатывается видеокартой \
одновременно. <i>Влияние:</i> чем выше значение, тем быстрее создание базы данных, но тем больше видеопамяти \
(VRAM) требуется. <i>Оптимально:</i> 16–32 для карт с 8 ГБ VRAM, 64–128 для карт с 12–24 ГБ. При ошибке \
&quot;Out of Memory&quot; (нехватка памяти) немедленно уменьшите это значение.</p>
<p><b>[batch CPU]</b> (от 1 до 64) Определяет количество фрагментов, обрабатываемых процессором за один раз. \
<i>Влияние:</i> увеличение значения ускоряет работу на CPU, но сильнее нагружает все ядра процессора и \
оперативную память (RAM). <i>Оптимально:</i> 4–8 для слабых ПК, 16–32 для современных многоядерных процессоров.</p>
<p><b>Рекомендация:</b> для большинства задач оптимальна модель <b>paraphrase-multilingual</b> \
(batch CPU 16 / batch GPU 32). Для слабых компьютеров — <b>all-MiniLM-L6-v2</b> (batch CPU 4-8). \
Для максимальной точности на мощном ПК — <b>intfloat/multilingual-e5-large</b> или <b>BAAI/bge-m3</b> \
с максимальным batch GPU, который позволяет видеопамять.</p>
<p><b>ФУНКЦИОНАЛ:</b></p>
<p><img src = \"qrc:/resources/images/DCButtons/24x24/DCKnopkaNazad.png\"> - Вернуться в предыдущее меню \
(Alt+стрелка влево).</p>
<p><img src = \"qrc:/resources/images/DCButtons/24x24/DCKnopkaInfo.png\"> - Помощь (F1).</p>
<p>После изменения параметров рекомендуется пересоздать базу данных для применения новых настроек.</p>
            		</body>
        		</html>"
    		); 
