...
<p><b>[💾 Сохранить результат]</b> - Вы можете сохранить результат анализа в текстовом файле.(Ctr+S).</p>
            ...
            txdZona.text = qsTr("
                <html>
                    <body>
<p>На данной странице вы можете настроить качество и скорость анализа документов с помощью языковых моделей.</p>
<p><b>НАСТРОЙКА НЕЙРО АНАЛИЗА:</b></p>
<ol>
<p><b>[температура]</b> Настройка параметра Temperature. Этот параметр влияет на уровень креативности \
ответов языковой модели. Значение 0.5 является сбалансированным. Чем выше значение, тем более непредсказуемы \
и творчески ответы; чем ниже — тем они строже и точнее.</p>
<p><b>[Перекрытие]</b> Качество анализа увеличивается с ростом параметра перекрытия. \
Скорость анализа замедляется при увеличении параметра парекрытия.</p>
<p>0%	⭐⭐ (60%)	Чанки изолированы, границы рвут контекст. Если ключевая мысль разрезана между чанками \
— модель её не увидит.</p>
<p>10%	⭐⭐⭐ (75%)	Минимальная защита от разрыва мысли. Работает для простых текстов (новости, FAQ).</p>
<p>20%	⭐⭐⭐⭐ (85%)	Оптимально для большинства задач. Предложения на границах полностью попадают в \
соседний чанк.</p>
<p>30%	⭐⭐⭐⭐⭐ (92%)	Для сложных текстов (научные статьи, юридические документы). \
Абзацы гарантированно перекрываются.</p>
<p>40%	⭐⭐⭐⭐⭐ (94%)	Почти нет потерь контекста, но огромные затраты. Только для критичных задач.</p>
</ol>
<p><b>ФУНКЦИОНАЛ:</b></p>
<ol>
<p><img src = \"qrc:/resources/images/DCButtons/24x24/DCKnopkaNazad.png\"> - Вернуться в предыдущее меню \
