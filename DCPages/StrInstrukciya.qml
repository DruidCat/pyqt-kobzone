...
<p><b>[💾 Сохранить результат]</b> - Вы можете сохранить результат анализа в текстовом файле.(Ctr+S).</p>
            ...
            txdZona.text = qsTr("
                <html>
                    <body>
<p>На данной странице вы можете настроить параметры взаимодействия с локальным сервером нейросетей \
LM Studio.</p>
<p><b>УПРАВЛЕНИЕ LM STUDIO:</b></p>
<ol>
<p><b>[статус LM Studio]</b> Позволяет запустить или остановить работу локального сервера нейросети \
LM Studio. Кнопка меняет свое название в зависимости от текущего состояния.</p>
<p><b>[путь к LM Studio]</b> Позволяет указать путь к исполняемому файлу программы LM Studio на вашем \
компьютере.</p>
<p><b>[выбор модели]</b> Позволяет выбрать конкретную языковую модель из списка тех, что были предварительно \
загружены в LM Studio.</p>
...
<p><b>[максимальный контекст]</b> Размер памяти модели — объём текста (в токенах), который нейросеть \
одновременно удерживает при ответе: ваш вопрос, найденные фрагменты документов и сам ответ. Чем больше \
значение, тем больше информации модель видит за раз и тем точнее ответ, но тем выше нагрузка на память \
и ниже скорость работы. Рекомендуемое значение — 22016 токенов. При ошибках нехватки памяти — уменьшите \
параметр; при анализе длинных документов — увеличьте.</p>
<p>Качество анализа (↑ с ростом overlap)
Overlap	Качество	Пояснение
0%	⭐⭐ (60%)	Чанки изолированы, границы рвут контекст. Если ключевая мысль разрезана между чанками — модель её не увидит.
10%	⭐⭐⭐ (75%)	Минимальная защита от разрыва мысли. Работает для простых текстов (новости, FAQ).
20%	⭐⭐⭐⭐ (85%)	Оптимально для большинства задач. Предложения на границах полностью попадают в соседний чанк.
30%	⭐⭐⭐⭐⭐ (92%)	Для сложных текстов (научные статьи, юридические документы). Абзацы гарантированно перекрываются.
40%	⭐⭐⭐⭐⭐ (94%)	Почти нет потерь контекста, но огромные затраты. Только для критичных задач.</p>
</ol>
<p><b>ФУНКЦИОНАЛ:</b></p>
<ol>
<p><img src = \"qrc:/resources/images/DCButtons/24x24/DCKnopkaNazad.png\"> - Вернуться в предыдущее меню \
...
<p>Если LM Studio не запущена или модель не выбрана, процесс анализа работать не будет.</p>
            ...
            txdZona.text = qsTr("
                <html>
                    <body>
<p><b>Исправление текста</b> — это анализ текста прошедшено через транскрибацию и исправление орфографии.</p>
                    </body>
                </html>"
            );
